#!/usr/bin/env python3
"""
Генерация vars/pgsql_secrets.yml: случайные пароли для каждого сайта из php_sites
и для all_db_user (PostgreSQL).

Зависимость: pip install pyyaml

Использование (из каталога ansible-php-project):
  python3 scripts/generate_pgsql_secrets.py              # создать или дополнить файл
  python3 scripts/generate_pgsql_secrets.py --force-all  # заново сгенерировать все пароли
  python3 scripts/generate_pgsql_secrets.py --dry-run    # только вывод в stdout

После смены паролей в файле снова запустите:
  ansible-playbook playbooks/deploy_postgresql.yml
модуль postgresql_user обновит пароли на сервере идемпотентно.
"""
...
def pw() -> str:
    # Достаточно для SCRAM-SHA-256; без проблемных символов для YAML/кавычек
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Сгенерировать vars/pgsql_secrets.yml")
    parser.add_argument(
        "--project-root",
        type=Path,
    ...
    out_file = root / "vars" / "pgsql_secrets.yml"
    ...
    for row in sites:
        if not (isinstance(row, dict) and "name" in row):
            continue
        db = row.get("db")
        if db is None or str(db) in {"pgsql", "both"}:
    ...
    if out_file.is_file() and not args.force_all:
        ...
        raw = sec.get("pgsql_site_passwords")
        if isinstance(raw, dict):
            existing_pw = {str(k): str(v) for k, v in raw.items()}
        a = sec.get("postgresql_all_db_user_password")
    ...
    body = yaml.safe_dump(
        {"pgsql_site_passwords": site_passwords, "postgresql_all_db_user_password": all_db_pw},
        allow_unicode=True,
        default_flow_style=False,
        sort_keys=False,
    )
    header = (
        "# Файл с секретами PostgreSQL — в .gitignore.\n"
        "# Регенерация: python3 scripts/generate_pgsql_secrets.py --force-all\n\n"
    )
    output = header + body
